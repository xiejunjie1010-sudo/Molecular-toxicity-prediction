#!/usr/bin/env python
"""
HGT on Tox21 —— 5-fold CV
输出格式同 R-GCN:
  detail_fold*.csv, metrics_fold*.csv,
  summary_per_receptor.csv, summary_macro.csv
运行命令：a
...
# >>> 受体缩写顺序必须与 label_matrix.npy 列一致
RECEPTORS = [
...
ap.add_argument('--save_dir',  default='../HGT/save/kg_fp5_HGT')
...
# ───────── chem_type ─────────
if args.chem_type >= 0:
    CHEM_T = args.chem_type
else:
    CHEM_T = np.bincount(node_type[(labels_np!=-1).any(1)].numpy()).argmax()
...
for o_tr,te in outer.split(np.zeros(len(chem_idx)),y_bin):
    inner=MultilabelStratifiedShuffleSplit(
        n_splits=100,test_size=0.2,random_state=args.seed)
...
for f,(tr,va,te) in enumerate(folds):
    ...
    for ep in tqdm(range(1,args.epochs+1),desc=f'Fold{f}',ncols=80):
        ...
        y_tr  =y_full.index_select(0,tr_d); mask=(y_tr!=-1).float()
        y_hat=y_tr.clone(); y_hat[y_hat==-1]=0
        loss=F.binary_cross_entropy_with_logits(out_tr,y_hat,weight=mask,reduction='sum')/mask.sum()
    ...
    with torch.no_grad(): te_p=torch.sigmoid(model(x_dict_cpu)).index_select(0,te_r).cpu().numpy()
...
print(f"[INFO] 所有结果已保存到 {SAVE_DIR.resolve()}")
