"""
GCN + SMOTE-XAttr-Avg 5-fold CV baseline
│– 指标：BAC / F1 / AUC
│– 输出：ROC_<task>.jpg  metrics_<task>.csv
│– 进度：tqdm
"""
#对比gcn-bac1加入了smote过采样，并且对12个受体做了预测实验
...
from smote_fp_gcn import SmoteXAttrAvg               # 保持原用法
...
save_dir = Path('./save/smote_x_attr_more_avg_5_gcn_bac')
...
def eval_metrics(y_true, y_prob, thr: float = 0.5):
    """返回 BAC, F1, AUC 三指标"""
...
def k_fold_split(ds, k=5):
    ...
    fold = len(ds)//k
    splits = [idx[i*fold:(i+1)*fold] for i in range(k)]
    trains, tests = [], []
    for i in range(k):
        tests.append([ds[j] for j in splits[i]])
        train_idx = [j for f in range(k) if f!=i for j in splits[f]]
        trains.append([ds[j] for j in train_idx])
    return trains, tests
...
class GCN(torch.nn.Module):
    def __init__(self, in_c, hid, out_c):
        super().__init__()
        self.conv1 = GCNConv(in_c, hid,        cached=False, normalize=not args.use_gdc)
        self.conv2 = GCNConv(hid,  hid*2,      cached=False, normalize=not args.use_gdc)
...
for task in tasks:
    ...
    valid = [bool(Chem.MolFromSmiles(s)) for s in df['Smiles']]
    num   = sum(valid)
    ...
    for fold in range(5):
        ...
        # 模型 & 优化器
        model = GCN(args.num_features, args.hidden_channels, args.num_classes).to(device)
        seed_all(105+fold)
