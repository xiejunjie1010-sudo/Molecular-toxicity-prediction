...
* CLI 追加 --pw_max 控制 pos_weight 上限，默认 2.0
* 发现 NaN 立即 sys.exit()
* 输出 detail_fold*.csv / metrics_fold*.csv / summary_*.csv

python train_gps_gine_cv.py --epochs 1000 --hidden 128 --heads 4
"""

import argparse, random, gc, sys, numpy as np, torch, pandas as pd
...
ap.add_argument('--pw_max',  type=float, default=2.0,  help='pos_weight 最大值')
...
SAVE = Path('../save/kg_fp_fold5_GPS-GINE_pos_weight5'); SAVE.mkdir(parents=True, exist_ok=True)
...
# ───────────── 5-fold 划分 ─────────────
folds=[]
for trva, te in MultilabelStratifiedKFold(
        5, shuffle=True, random_state=args.seed).split(np.zeros(len(chem_idx)), y_chem_np):
...
def train_fold(fid, masks):
    ...
    for ep in tqdm(range(1,args.epochs+1), desc=f'Fold{fid}'):
        ...
        if torch.isnan(loss):
            sys.exit(f'NaN detected at epoch {ep}. Terminating run.')
