#!/usr/bin/env python
"""
5-fold R-GCN ⊕ 基因/通路可训练嵌入
保存位置：R-GCN/save/kg_fp_fold5/
...
SAVE_DIR = Path('../save/kg_fp_fold5_12_yn'); SAVE_DIR.mkdir(parents=True, exist_ok=True)
...
def per_receptor_metrics(y_true, y_prob):
    ...
    for k in range(12):
        ...
        if m.sum()==0:
            ...
            col_tpr.append(np.nan); col_tnr.append(np.nan)  # >>> 补 NaN
            continue
        ...
        tpr = tp / (tp + fn + 1e-9)      # 灵敏度 = 有毒准确率
        tnr = tn / (tn + fp + 1e-9)      # 特异度 = 无毒准确率
        col_tpr.append(tpr); col_tnr.append(tnr)
...
def train_fold(fold, masks):
    model = RGCN(in_dim, args.hidden, num_rel,
...
# 5-fold 平均（逐受体，含 Toxic/NonToxic）
