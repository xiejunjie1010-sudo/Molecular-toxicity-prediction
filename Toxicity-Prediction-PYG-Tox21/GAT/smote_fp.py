...
class SmoteXAttrMore:
    ...
    def fit(self, datasets):
        ...
        print("smote_x_attr_more:k=%d nontox_idx:%d tox_idx:%d" % (self.k, len_negative, len_positive))
        ...
        if len_positive == len_negative:
            ...
            print("smote_x_attr:sampling_rate=%d nontox_idx:%d tox_idx:%d" % (
...
class SmoteSimilarityAllFPXAttrMore:
    """
    SMOTE过采样算法.
    ...
    def fit(self, datasets,mols):
        ...
        print("smote_fp_all_more:k=%d nontox_idx:%d tox_idx:%d" % (self.k, len_negative, len_positive))
        ...
        if len_positive == len_negative:
            ...
            self.less = len_positive
            self.sampling_rate = 1 +(len_negative - len_positive) // len_positive
            print("smote_fp_all_more:sampling_rate=%d nontox_idx:%d tox_idx:%d" % (self.sampling_rate, len_negative, len_positive * (self.sampling_rate + 1)))
            self.get_samples(positive, positive_mols)
            return self.datasets_smote
        else:
            self.more = len_positive
            self.less = len_negative
            self.sampling_rate = 1 + (len_positive - len_negative) // len_negative
            print("smote_fp_all_more:sampling_rate=%d nontox_idx:%d tox_idx:%d" % (self.sampling_rate, len_negative * (self.sampling_rate + 1), len_positive))
            self.get_samples(negative, negative_mols)
            return self.datasets_smote
