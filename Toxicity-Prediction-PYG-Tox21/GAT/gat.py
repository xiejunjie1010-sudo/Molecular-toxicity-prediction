...
for filename in raw_file_names:
    ...
    for i in range(folds):
        ...
        # train_data[i] = random_over_sampling(train_data[i])
        if args.use_gdc:
            transform = T.GDC(
                self_loop_weight=1,
    ...
    plt.savefig('./save/none_5_gat/ROC_%s.jpg' % filename)
    # plt.show()
    plt.close()
    # mean_aucs.append(mean_auc)
    # mean_accs.append(np.mean(score))
    print("acc", filename, ":", np.mean(score))
    np.save('./save/none_5_gat/auc_%s.npy' % filename, aucs)
    np.save('./save/none_5_gat/acc_%s.npy' % filename, score)
