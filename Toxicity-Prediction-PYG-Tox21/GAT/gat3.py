...
def k_fold_cross_validation_split_similarity(datasets, fps, folds=5):
    basket_split_data = list()
    basket_split_fps = list()
    fold_size = int(len(datasets) / folds)
    idx = list(range(len(datasets)))

    while len(basket_split_data) < folds:
        basket_random_fold_data = list()
        basket_random_fold_fps = list()
        for i in range(fold_size):
            random_choose_index = np.random.choice(idx)
            basket_random_fold_data.append(datasets[random_choose_index])
            basket_random_fold_fps.append(fps[random_choose_index])
            idx.remove(random_choose_index)
        basket_split_data.append(basket_random_fold_data)
        basket_split_fps.append(basket_random_fold_fps)
    train_fps = list()
    train_data = list()
    test_data = list()
    for i in range(folds):
        ...
        train_fps_temp = list()
        for j in temp:
            train_data_temp.extend(basket_split_data[j])
            train_fps_temp.extend(basket_split_fps[j])
        train_data.append(train_data_temp)
        train_fps.append(train_fps_temp)
    return train_data, test_data, train_fps
...
for filename in raw_file_names:
    ...
    train_data, test_data, train_mols = k_fold_cross_validation_split_similarity(datasets, mols, folds)
    ...
    for i in range(folds):
        ...
        smote = SmoteSimilarityAllFPXAttrAvgGCN(k=5)  # Expected n_neighbors <= n_samples
        train_data[i] = smote.fit(train_data[i], train_mols[i])
    ...
    plt.savefig('./save/smote_fp_all_XAttr_more_avg_5_gat/ROC_%s.jpg' % filename)
    # plt.show()
    plt.close()
    # mean_aucs.append(mean_auc)
    # mean_accs.append(np.mean(score))
    print("acc", filename, ":", np.mean(score))
    np.save('./save/smote_fp_all_XAttr_more_avg_5_gat/auc_%s.npy' % filename, aucs)
    np.save('./save/smote_fp_all_XAttr_more_avg_5_gat/acc_%s.npy' % filename, score)
