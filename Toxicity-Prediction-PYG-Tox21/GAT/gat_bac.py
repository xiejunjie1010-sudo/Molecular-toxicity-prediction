"""
GAT 5-fold CV baseline
│─ 指标：BAC / F1 / AUC
│─ 结果：./save/none_5_gat/metrics_<task>.csv、ROC_<task>.jpg
│─ 进度：tqdm
"""
...
save_dir = Path('./save/none_5_gat_bac'); save_dir.mkdir(parents=True, exist_ok=True)
    ...
    for fold in range(5):
        ...
        if args.use_gdc:
            ...
            for idx_d, d in enumerate(train_sets[fold]):
                d.edge_attr = None            # 删除旧 edge_attr
